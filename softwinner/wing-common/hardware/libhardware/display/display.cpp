...
static int display_postFb(struct display_device_t *dev){
    struct display_context_t* ctx = (struct display_context_t*)dev;
    if(ctx->cur_3dmode[0]){
        unsigned long               args[4] = {0};
        __disp_layer_info_t         layer_para;
        __u32                       layer_hdl;
        __disp_rect_t               fb_rect;

        ioctl(ctx->mFD_fb[0], FBIOGET_LAYER_HDL_0, &layer_hdl);

        args[0] = 0;
        args[1] = (__u32 ) layer_hdl;
        args[2] = (__u32 ) &layer_para;
        ioctl(ctx->mFD_disp, DISP_CMD_LAYER_GET_PARA, (unsigned long) args);

        fb_rect = layer_para.src_win;

        memset(&layer_para, 0 , sizeof(layer_para));

        args[0] = 0;
        args[1] = (__u32 ) ctx->hw_3d_layer_handler[0];
        args[2] = (__u32 ) &layer_para;
        ioctl(ctx->mFD_disp, DISP_CMD_LAYER_GET_PARA, (unsigned long) args);

        layer_para.src_win = fb_rect;

        args[0] = 0;
        args[1] = (__u32) ctx->hw_3d_layer_handler[0];
        args[2] = (__u32) &layer_para;
        ioctl(ctx->mFD_disp, DISP_CMD_LAYER_SET_PARA, (unsigned long) args);
    }
    return 0;
}
...
static int display_set3dlayeroffset(struct display_device_t *dev, int displayno, int left, int right)
{
    ...
    if(ctx->cur_3dmode[displayno]){
        ...
        left = (ctx->hw_3d_layer_offset_l[displayno] * ctx->area_percent[displayno]) / 100;
        right = (ctx->hw_3d_layer_offset_r[displayno] * ctx->area_percent[displayno]) / 100;
        ...
        args[0] = displayno;
        args[1] = (__u32 ) ctx->hw_3d_layer_handler[displayno];
        args[2] = (__u32 ) &layer_para;
        ioctl(ctx->mFD_disp, DISP_CMD_LAYER_GET_PARA, (unsigned long) args);

        layer_para.scn_win.x = (ctx->width[displayno] - ctx->valid_width[displayno])/2 + right;;
        layer_para.scn_win.y = (ctx->height[displayno] - ctx->valid_height[displayno])/2 + 1080;
        layer_para.scn_win.width = ctx->width[displayno];
        layer_para.scn_win.height = ctx->height[displayno];

        LOGD("####%s right scn[%d,%d,%d,%d]",__func__,
                layer_para.scn_win.x,layer_para.scn_win.y,
                layer_para.scn_win.width,layer_para.scn_win.height);
...
static int display_set3dmode(struct display_device_t *dev,int displayno,int cur_3denable)
{
    ...
    else if(1 == cur_3denable && !ctx->cur_3dmode[displayno])
    {
        ...
        // request a new hw layer
        args[0] = displayno;
        args[1] = (__u32) DISP_LAYER_WORK_MODE_NORMAL;
        ctx->hw_3d_layer_handler[displayno] = ioctl(ctx->mFD_disp, DISP_CMD_LAYER_REQUEST,
                (unsigned long) args);
        if (ctx->hw_3d_layer_handler[displayno] == 0) {
            LOGE("Request HW layer failed!");
        }

        // set hw layer parameter
        memset(&layer_para, 0, sizeof(__disp_layer_info_t ));
        ...
        layer_para.fb.size.width = ctx->width[0];
        layer_para.fb.size.height = ctx->height[0];
        ...
        struct fb_fix_screeninfo    fix_fb;
        ioctl(ctx->mFD_fb[displayno], FBIOGET_FSCREENINFO, &fix_fb);
        layer_para.fb.addr[0] = fix_fb.smem_start;
        ...
        args[0] = displayno;
        args[1] = (__u32 ) ctx->hw_3d_layer_handler[displayno];
        ioctl(ctx->mFD_disp, DISP_CMD_LAYER_OPEN, (unsigned long) args);
    ...
    else if(0 == cur_3denable && ctx->cur_3dmode[displayno])
    {
        ...
        args[0] = displayno;
        args[1] = (__u32 ) ctx->hw_3d_layer_handler[displayno];
        ioctl(ctx->mFD_disp, DISP_CMD_LAYER_CLOSE, (unsigned long) args);

        args[0] = displayno;
        args[1] = (__u32 ) ctx->hw_3d_layer_handler[displayno];
        ioctl(ctx->mFD_disp, DISP_CMD_LAYER_RELEASE, (unsigned long) args);
        ...
        struct hw_device_t** device)
{
    ...
    ctx->device.postFb                  = display_postFb;
