        ...
        size_t numDisplays, hwc_display_contents_1_t** displays) 
{
    ...
    if (onlyOverlay && fb_stat == 0) {
        ...
        int g2d = open("/dev/g2d", O_RDWR, 0);
        if (g2d == NULL) {
            ALOGW("Can not open the g2d driver.");
        } else {
            struct fb_fix_screeninfo fix_fb;
            ioctl(sun4i_dev->mFD_fb[0], FBIOGET_FSCREENINFO, &fix_fb);
            struct fb_var_screeninfo var;
            ioctl(sun4i_dev->mFD_fb[0], FBIOGET_VSCREENINFO, &var);

            g2d_fillrect fillrect;
            fillrect.flag = G2D_FIL_NONE;
            fillrect.dst_image.addr[0] = fix_fb.smem_start; //output address
            fillrect.dst_image.w = var.xres;
            fillrect.dst_image.h = var.yres;
            fillrect.dst_image.format = G2D_FMT_ARGB_AYUV8888;
            fillrect.dst_image.pixel_seq = G2D_SEQ_NORMAL;
            fillrect.dst_rect.x = 0;
            fillrect.dst_rect.y = 0;
            fillrect.dst_rect.w = var.xres;
            fillrect.dst_rect.h = var.yres;
            fillrect.color = 0;
            fillrect.alpha = 0;
            if (ioctl(g2d, G2D_CMD_FILLRECT, &fillrect) < 0) {
                ALOGW("G2D_CMD_FILLRECT failed!\n");
            }
        }
        //gettimeofday(&end,NULL);
        //ALOGW("mem set zero the fb cost %d\n", (end.tv_usec - start.tv_usec)/1000);
        fb_stat = 1;
